	...
	export default {
		...
		created(){
			...
			services.get('auth/role',{
            	...
            	if (response.data.iTotal && response.data.data.update) {
            		...
            		services.get('role',{
		            	...
		            	this.forms.status 	= parseInt(row.status.id);
				    	...
				    	services.get('menu',{
	                        ...
	                        for(let i=0; i < this.menus.length; i++){
	                            ...
	                            {
	                                for(let n=0; n < row.menu.length; n++){
	                                    if(this.menus[i].id == row.menu[n].menu_id)
	                                    {
	                                        let role_menu = JSON.parse(row.menu[n].role_menu);
	                                        this.forms[action[x]][row.menu[n].menu_id] =  role_menu[action[x]];
	                                    }
	                                }
	                            }
	                        }
		...
		methods : {
			store(){ 
				...
				services.put('role/update',this.forms,{
		           	headers: {'KEY': this.token}
		       	}).then(response => {
		           	this.completed = true;
